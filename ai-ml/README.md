...
| | | | |
|---|---|---|---|
| [Accuracy（準確率）](terms/accuracy.md) | [Agent Anomaly Detection（代理異常偵測）](terms/agent-anomaly-detection.md) | [Agent Evaluation（代理評估）](terms/agent-evaluation.md) | [Agent Identity（代理身分）](terms/agent-identity.md) |
...
| [Model Routing（模型路由）](terms/model-routing.md) | [Multimodal（多模態）](terms/multimodal.md) | [Neural Network（神經網路）](terms/neural-network.md) | [NLP（自然語言處理）](terms/nlp.md) |
| [Overfitting（過擬合）](terms/overfitting.md) | | | |
...
| [Content Exclusion（內容排除）](terms/content-exclusion.md) | | | |
...
| Behavioral Evaluation（行為評估） | 檢查 Agent 是否做出正確、可觀察的中間行為，而不只看最後答案 | 驗收助理時也看他有沒有查證、請示與自我檢查 | 「先跑 Behavioral Evaluation，確認 Agent 真的有查資料」 | [詳細](terms/behavioral-evaluation.md) |
...
| Content Exclusion | 助理進辦公室前，先把機密文件鎖進櫃子 |
