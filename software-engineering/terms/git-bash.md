...
想像你人生地不熟去日本旅遊，但手上拿著一本「中文轉日文即時翻譯機」——路牌、菜單通通看得懂，不用現場現學日文。

Git Bash 對 Windows 使用者來說就是這台翻譯機：

- 別人寫的教學說「打開 Terminal，輸入 `ls` 看檔案列表」，Windows 使用者不用煩惱「我的電腦要打什麼指令」，打開 Git Bash 照樣打 `ls` 就對了
- 工程師在會議上說「你在 terminal 打一下這行指令」，Windows 使用者也能直接照做，不用來回問「這在 Windows 要怎麼打」
