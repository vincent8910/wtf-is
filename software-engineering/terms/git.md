...
- 你在打一款高難度遊戲，每過一個關卡就存檔一次。打 Boss 死掉，直接讀取上一個存檔重來，不用整個遊戲重新玩
- 寫文件寫到一半，Claude Code 幫你改壞了？用 Git 讀取「上一個存檔」，瞬間恢復原狀，不用重新打字
- 工程師常說「先 commit 再改」，意思就是「先存檔，再去嘗試冒險的東西」——出事了還能讀檔重來
